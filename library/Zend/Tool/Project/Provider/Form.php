...
class Zend_Tool_Project_Provider_Form extends Zend_Tool_Project_Provider_Abstract
{
    ...
    public static function createResource(Zend_Tool_Project_Profile $profile, $formName, $moduleName = null)
    {
        if (!is_string($formName)) {
            throw new Zend_Tool_Project_Provider_Exception('Zend_Tool_Project_Provider_Form::createResource() expects \"formName\" is the name of a forms resource to create.');
        }

        if (!($formsDirectory = self::_getFormsDirectoryResource($profile, $moduleName))) {
            if ($moduleName) {
                $exceptionMessage = 'A forms directory for module "' . $moduleName . '" was not found.';
            } else {
                $exceptionMessage = 'A forms directory was not found.';
            }
    ...
    {
        if (!is_string($formName)) {
            throw new Zend_Tool_Project_Provider_Exception('Zend_Tool_Project_Provider_Form::createResource() expects \"formName\" is the name of a forms resource to check for existence.');
        }
    ...
    public function create($name, $module = null)
    {
        ...
        if (self::hasResource($this->_loadedProfile, $name, $module)) {
            throw new Zend_Tool_Project_Provider_Exception('This project already has a forms named ' . $name);
        }
        ...
        if ($this->_registry->getRequest()->isPretend()) {
            ...
            $this->_registry->getResponse()->appendContent('Would create a forms at '  . $formResource->getContext()->getPath());

            if ($testFormResource) {
                $this->_registry->getResponse()->appendContent('Would create a forms test file at ' . $testFormResource->getContext()->getPath());
            }

        } else {

            $this->_registry->getResponse()->appendContent('Creating a forms at ' . $formResource->getContext()->getPath());
            $formResource->create();

            if ($testFormResource) {
                $this->_registry->getResponse()->appendContent('Creating a forms test file at ' . $testFormResource->getContext()->getPath());
